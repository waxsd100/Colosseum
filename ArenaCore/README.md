...
⑦ 賭け受付開始          /arena open
        ↓
⑧ 試合開始              /arena start               ← 待機場のプレイヤー自動登録
        ↓
⑨ 勝者宣言              /arena win Warriors         ← 配当自動配布・地形復元
```
...
> **💡 ポイント**
> - `create` でチームは作らない。`team add` で後から追加する
> - 待機場に立っているプレイヤーが `start` 時に自動でチーム登録される
> - プリセットを保存すれば、2回目以降は4コマンドで完結
...
| `/arena field set [名前]` | 選択範囲 or 保存済みフィールドを戦闘エリアとして設定 |
