...
1.  `/arena create <アリーナ名>` でセッションを開設。
2.  `/arena team add <チーム名>` でチームを追加。
3.  WorldEdit で待機場にしたい範囲を選択し、`/arena team area <チーム名>` で待機場を設定します。
4.  待機場が設定されると、**プレイヤーが待機場エリアに入るだけで自動的にチームに参加**し、エリアから出ると自動的にチームから離脱します。
5.  WorldEdit で選択した範囲を `/arena region <チーム名>` で賭けエリアとして設定します。
6.  `/arena open` で賭けの受付を開始し、試合開始時に `/arena start` で締め切ります。
7.  決着がついたら `/arena win <勝利チーム名>` を宣言すると、オッズに応じた配当金が自動でチップとして配布され、全プレイヤーのチップが自動換金されてアリーナランキングに記録されます（途中で中止したい場合は `/arena cancel` で全額返金されます）。

### 待機場システム

プレイヤーと Mob の混合が許容されている待機場では、面倒なコマンド操作は不要です。
...
*   `/arena field set [名前]` : WorldEdit の選択範囲、または保存済みフィールドを戦闘エリアとして登録します。登録時に Schematic ファイルが自動保存され、地形復元に使用されます。
*   `/arena field info` : 現在の戦闘エリアの座標・ブロック数・ワールド名を表示します。
*   `/arena field save <名前>` : 現在の戦闘エリアを名前付きで保存します。
*   `/arena field list` : 保存済み戦闘エリアの一覧を表示します。
*   `/arena field delete <名前>` : 保存済み戦闘エリアを削除します。
...
```bash
# 全プラグインのビルド（ChipLib, CasinoCore, ArenaCore の jar ファイルが生成されます）
./gradlew build
