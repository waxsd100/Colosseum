...
看板の1行目に `[arena]`、2行目にプリセット名を書くと **開始看板** になります（作成には管理者権限が必要）。
設置後は**誰でも**右クリックするだけでプリセットをロードして募集を開始できます。セッション稼働中・地形復元中は無効です。

看板から開始したセッションは**無人進行**になります:

*   募集締切 → ベット締切 → 試合開始まで自動で進行（タイマーは `default-betting-duration`、未設定時は 60秒）
*   参加者が2チームに満たない場合は募集を自動延長し、上限（2回）を超えると自動中止
*   勝敗は勝利条件に従って自動判定（`win-condition: manual` の場合のみ `/arena win` が必要）
*   試合終了後に**オートループは発動しません**（連続開催したい場合は `/arena loop true` を使用）
