...
*   **ArenaCore**  
    闘技場でのベッティングを行うためのプラグインです。プレイヤーは、あらかじめ設定されたエリアに物理的にチップ（カーペット）を置くことで、好きなチームに賭けることができます。**複数のチームに同時に賭けることも可能**です。「やっぱり賭けをやめたい」という時は、配布されたハサミでカーペットを壊すだけで、いつでも手元にチップが戻ってきます。
...
1.  /arena create \<アリーナ名\> \<チーム名1\> \<チーム名2\>... でセッションを開設。セッション作成と同時に、Minecraft のバニラ Scoreboard Team（/team）も自動で作成されます。
2.  WorldEdit で待機場にしたい範囲を選択し、/arena region team \<チーム名\> で待機場を設定します。
3.  待機場が設定されると、**プレイヤーが待機場エリアに入るだけで自動的にチームに参加**し、エリアから出ると自動的にチームから離脱します。手動で /arena team add する必要はありません。
4.  WorldEdit で選択した範囲を /arena region bet \<チーム名\> で賭けエリアとして設定します。
5.  /arena open で賭けの受付を開始し、試合開始時に /arena start で締め切ります。
6.  決着がついたら /arena win \<勝利チーム名\> を宣言すると、オッズに応じた配当金が自動でチップとして配布されます（途中で中止したい場合は /arena cancel で全額返金されます）。

### 待機場システム

プレイヤーは待機場エリアに入るだけでチームに参加でき、離れると自動で離脱します。面倒なコマンド操作は不要です。
...
*   **/arena status で確認**: 現在のセッション情報に加え、各チームの待機場にいるプレイヤー名やMob数、TP先の設定状況がリアルタイムで表示されます。
...
*   house-edge: 運営の手数料（デフォルト 0.1 = 10%、有効範囲 0〜0.99）
...
*   ArenaCore/build/libs/ArenaCore-1.1.0.jar

### テスト構成

全 3 モジュールで 415 件以上のテストが実行されます。
